...
from utils import MALAnime,LoadCSV
...
# load dataframe
dataset = LoadCSV(root='dataset/')
total_dataframe, train_dataframe, test_dataframe = dataset.split_train_test()

# make torch.utils.data.Data object
train_set = MALAnime(df=train_dataframe,total_df=total_dataframe,ng_ratio=4)
test_set = MALAnime(df=test_dataframe,total_df=total_dataframe,ng_ratio=99)

# get number of unique userID, unique  movieID
max_num_users,max_num_items = total_dataframe['userId'].max()+1, total_dataframe['animeId'].max()+1
