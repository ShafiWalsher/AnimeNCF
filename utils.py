...
class LoadCSV():
    def __init__(self,
        ...
        self.fname = os.path.join(self.root,  'ratings_reduced.csv')
        # else:
    ...
    #     print('Downloading MALAnime dataset')
    ...
    def split_train_test(self) -> (pd.DataFrame, pd.DataFrame, pd.DataFrame):
        ...
        # train_dataframe, test_dataframe = random_split(self.df, [train_size, test_size])

        train_dataframe, test_dataframe = train_test_split(self.df, test_size=0.2, random_state=42)
        ...
        # train_dataframe['rating'] = 1
        # test_dataframe['rating'] = 1
...
class MALAnime(Dataset):
    ...
    def _negative_sampling(self) :
        ...
        users, items, labels = [], [], []
        user_item_set = set(zip(df['userId'], df['animeId']))
        total_user_item_set = set(zip(total_df['userId'],total_df['animeId']))
        all_animeIds = total_df['animeId'].unique()
